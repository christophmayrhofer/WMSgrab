...
INIRead, INI_UsePosY, settings.ini, Startup, UsePosY, 0
...
Gui, 5:Add, Edit, vullon x10 y40 w80 h20, Left Longitude
Gui, 5:Add, Edit, vlrlon x100 y40 w80 h20, Right Longitude
Gui, 5:Add, Edit, vullat x50 y10 w80 h20, Upper Latitude
Gui, 5:Add, Edit, vlrlat x50 y70 w80 h20, Lower Latitude

Gui, 5:Add, Radio, vsat x10 y110 w80 h20 Checked, SAT
Gui, 5:Add, Radio, vmap x10 y135 w80 h20 , MAP
Gui, 5:Add, Radio, vhyb x10 y160 w80 h20 , HYBRID
Gui, 5:Add, Radio, vair x10 y185 w80 h20 , AIR

Gui, 5:Add, Edit, vres x100 y110 w40 h20 , 1
Gui, 5:Add, Text, x140 y115 w50 h20 , Res. (m)
Gui, 5:Add, DropDownList, vformat x100 y135 w45 Choose1, JPG|PNG|BMP|GIF|ECW|IMG|ENVI|GeoTiff
Gui, 5:Add, Text, x145 y140 w35 h20 , Format
Gui, 5:Add, Button, x100 y165 w80 h20 , OK
Gui, 5:Add, Button, x100 y185 w80 h20 , Cancel

Gui, 2:+Owner1 -MaximizeBox -MinimizeBox
Gui, 2:add, Groupbox, x10 y10 w330 h80, Koordinaten
Gui, 2:add, text, xp+10 yp+30 w150, Längengrad (Lon):
Gui, 2:add, text, x+10 yp wp, Breitengrad (Lat):
...
Gui, 4:add, text, x84 y10 w226, "WMSgrab" based on`n"OpenStreetMap Viewer" 
Gui, 4:add, text, x10 y90 w150 +right, Programmcode (WMSgrab)
Gui, 4:add, text, xp y+5 wp +right, Programmcode (OSM Viewer)
...
Gui, 4:add, text, x170 y90 w150, Chris May
Gui, 4:add, text, x170 y+5 w150 gLinkAHK, Bentschi (AutoHotkey Forum)
...
hud_download := LoadTexture("icon\download.png")
...
WM_LBUTTONDOWN(wParam, lParam, msg, hwnd)
{
  ...
  if ((MouseY > (ScreenH/2)-22) && (MouseX > -105) && (MouseX < 105))
  {
    ...
    {
      GuiControl,5:,ullon, %LonUL%
      GuiControl,5:,lrlon, %LonLR%
      GuiControl,5:,ullat, %LatUL%
      GuiControl,5:,lrlat, %LatLR%
      Gosub, DownloadWindow
    }
    return 1
  }
...
WM_MOUSEMOVE(wParam, lParam, msg, hwnd)
{
  ...
  Lon := LonOpenGLToMap(MapPosX-((((ScreenW/2)/(ScreenW/2))-1)*(ScreenW/2)/(2**MapZoom)))
  Lat := LatOpenGLToMap(MapPosY-((((ScreenH/2)/(ScreenH/2))-1)*(ScreenH/2)/(2**MapZoom)))
  LonUL := LonOpenGLToMap(MapPosX-(((0/(ScreenW/2))-1)*(ScreenW/2)/(2**MapZoom)))
  LatUL := LatOpenGLToMap(MapPosY-(((0/(ScreenH/2))-1)*(ScreenH/2)/(2**MapZoom)))
  LonLR := LonOpenGLToMap(MapPosX-(((ScreenW/(ScreenW/2))-1)*(ScreenW/2)/(2**MapZoom)))
  LatLR := LatOpenGLToMap(MapPosY-(((ScreenH/(ScreenH/2))-1)*(ScreenH/2)/(2**MapZoom)))
...
else
{
  ...
  DrawIcon(87, ScreenH/2-20, hud_download)
}
...
if size > %exp6%
{
	...
	getformat = %A_FormatFloat%
	SetFormat, float, 0.0
	x -= 0
	y -= 0
	SetFormat, float, 0.2
	size -= 0
	SetFormat, float, %getformat%
		
	MsgBox,4,, Output size: %x% * %y% = %size% %exptxt% `n`nThis might take a while to download: `n0-100MP --> minutes`n100MP - 1GP --> hours`n>1GP not recommended`n`nYou may reduce the area or resolution --> Click No `nIf you want to keep your settings --> Click Yes `n`nContinue?
	IfMsgBox No
		Gosub, DownloadWindow
}
...
GDAL\gdal_translate %xml% %file%.%format% %outf% -outsize %x% %y% -projwin %ulx% %uly% %lrx% %lry% -a_srs EPSG:900913
